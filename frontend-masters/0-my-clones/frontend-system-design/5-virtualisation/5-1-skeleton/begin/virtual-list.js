import { intersectionObserver } from "./utils-obeserver/observer.js";
/**
...
export class VirtualList {
	...
	toHTML() {
		...
		<div id="container" >
		  ...
		  <main id="virtual-list"></main>
			<div id="bottom-observer">Botoom Observer</div>
		</div>
	...
	async #handleBottomObserver() {
		...
		if (this.pool.length < this.limit) {
			...
			const [toResycle, unchanged] = [
				this.pool.slice(0, this.props.pageSize),
				this.pool.slice(this.props.pageSize),
			];
			this.pool = unchanged.concat(toResycle);
			this.#updateData(toResycle, data);
			this.start++;
		}
	...
	async #handleTopObserver() {
		...
		const [unchanged, toResycle] = [
			this.pool.slice(0, this.props.pageSize),
			this.pool.slice(this.props.pageSize),
		];

		this.pool = toResycle.concat(unchanged);
		this.#updateData(toResycle, data);
	...
	#updateData(elements, data) {
		data.forEach((datum, index) => {
			this.props.updateTemplate(data[index], elements[index]);
		});
	}
