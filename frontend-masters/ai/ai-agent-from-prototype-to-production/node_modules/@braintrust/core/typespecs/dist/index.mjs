...
function generateBaseEventOpSchema(objectType) {
  ...
  return z12.object({
    ...
    span_attributes: z12.object({
      name: z12.string().nullish().describe("Name of the span, for display purposes only"),
      type: z12.enum(spanTypeAttributeValues).nullish().describe("Type of the span, for display purposes only")
    }).catchall(customTypes.unknown).nullish().describe(
...
var pathTypeFilterSchema = z12.object({
  type: z12.literal("path_lookup").describe("Denotes the type of filter as a path-lookup filter"),
  path: z12.string().array().describe(
...
var mergeEventSchema = z12.object({
...
var replacementEventSchema = z12.object({
...
function makeInsertEventSchemas(objectType, insertSchema) {
  ...
  const eventSchema = z12.union([replaceVariantSchema, mergeVariantSchema]).describe(`${capitalize(article)} ${eventDescription} event`).openapi(`Insert${eventSchemaName}Event`);
...
var attachmentReferenceSchema = z13.object({
  type: z13.literal(BRAINTRUST_ATTACHMENT),
  filename: z13.string().min(1),
  content_type: z13.string().min(1),
  key: z13.string().min(1)
});
var uploadStatusSchema = z13.enum(["uploading", "done", "error"]);
var attachmentStatusSchema = z13.object({
  upload_status: uploadStatusSchema,
  error_message: z13.string().nullish().transform((x) => x || void 0)
});
export {
