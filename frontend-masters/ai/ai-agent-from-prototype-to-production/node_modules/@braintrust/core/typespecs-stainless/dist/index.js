...
__export(typespecs_exports, {
  ...
  fetchEventsFiltersSchema: () => fetchEventsFiltersSchema,
...
function generateBaseEventOpSchema(objectType) {
  ...
  return import_zod12.z.object({
    ...
    span_attributes: import_zod12.z.object({
      name: import_zod12.z.string().nullish().describe("Name of the span, for display purposes only"),
      type: import_zod12.z.enum(spanTypeAttributeValues).nullish().describe("Type of the span, for display purposes only")
    }).catchall(customTypes.unknown).nullish().describe(
...
var pathTypeFilterSchema = import_zod12.z.object({
  type: import_zod12.z.literal("path_lookup").describe("Denotes the type of filter as a path-lookup filter"),
  path: import_zod12.z.string().array().describe(
...
var fetchEventsFiltersSchema = pathTypeFilterSchema.array().describe(
  [
...
var mergeEventSchema = import_zod12.z.object({
...
var replacementEventSchema = import_zod12.z.object({
...
function makeInsertEventSchemas(objectType, insertSchema) {
  ...
  const eventSchema = import_zod12.z.union([replaceVariantSchema, mergeVariantSchema]).describe(`${capitalize(article)} ${eventDescription} event`).openapi(`Insert${eventSchemaName}Event`);
...
var attachmentReferenceSchema = import_zod13.z.object({
  type: import_zod13.z.literal(BRAINTRUST_ATTACHMENT),
  filename: import_zod13.z.string().min(1),
  content_type: import_zod13.z.string().min(1),
  key: import_zod13.z.string().min(1)
});
var uploadStatusSchema = import_zod13.z.enum(["uploading", "done", "error"]);
var attachmentStatusSchema = import_zod13.z.object({
  upload_status: uploadStatusSchema,
  error_message: import_zod13.z.string().nullish().transform((x) => x || void 0)
});
